...
\input{codes/top_detection_adaptive_radius.tex}

Wie in der Implementierung zu sehen, werden die theoretischen Modelle der Formeln \ref{eq:allometric_radius} und \ref{eq:nms_distance} direkt miteinander verzahnt. Der theoretische Kronenradius (\textit{crown\_rad}) wird für jeden identifizierten Maximum-Kandidaten berechnet und zusammen mit den räumlichen Pixel-Koordinaten (\textit{row, col}) in ein Dictionary-Objekt abgelegt. Nach der absteigenden Sortierung der Liste \textit{candidates} durchläuft jeder Punkt die euklidische Distanzprüfung. Ein Baumkandidat wird verworfen, sobald sein mit der Rasterauflösung (\textit{res}) multiplizierter Abstand zu einem bereits akzeptierten Baum dessen indiviuellen Radius und die Konstante \textit{nms\_overlap\_factor} unterschreitet. Die verbleibenden, validierten Maxima werden unter Verwendung von GeoPandas als strukturierte Vektordaten aufbereitet und unter Beibehaltung des Koordinatebezugssystems in GPKG-Datei gespeichert.